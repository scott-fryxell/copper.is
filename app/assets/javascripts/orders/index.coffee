$(document).on "load.orders_index", ->
  $('*[itemtype="tips"] form').each ->
    ...
    dollars = document.copper.cents_to_dollars(cents)
    $(@).find("input[itemprop=amount_in_dollars]").attr('value', dollars)
  ...
  $("*[itemtype=tips] form[method=delete]").on 'item.delete', ->
    ...
    $(order).find('section > header > h4 > span').text document.copper.cents_to_dollars(new_total) 
