module Authorizable
  ...
  module ClassMethods
    ...
    def subclass_as authorizer_name
      ...
      ("Authorizer::" + authorizer_name.to_s.capitalize).constantize
    end
    ...
    def authorizer_name url
      ...
      when /facebook\.com$/ then
        Authorizer::Facebook.filter_url uri
      when /tumblr\.com$/ then
        Authorizer::Tumblr.filter_url uri
      when /twitter\.com$/ then
        Authorizer::Twitter.filter_url uri
      when /plus\.google\.com$/ then
        'google'
      when /vimeo\.com$/ then
        Authorizer::Vimeo.filter_url uri
      when /flickr\.com$/ then
        'flickr'
      when /github\.com$/ then
        Authorizer::Github.filter_url uri
      when /youtube\.com$/ then
        'youtube'
      when /soundcloud\.com$/ then
        Authorizer::Soundcloud.filter_url uri
